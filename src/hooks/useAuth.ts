...
const fetchToken = async (code: string | null, signal: AbortSignal, handle: ((...args: undefined[]) => void)[]) => {
    if (!code) return;

    const [setAT, setET, setRT] = handle;

    isFetchingToken = true;
    await axios.post(url, { code }, { signal })
    .then((res) => {
...
export default function useAuth(code: string | null) {
    ...
    useEffect(() => {
        ...
        if (!isFetchingToken) {
            fetchToken(code, signal, [setAccessToken, setExpiresTime, setRefreshToken]);
        }
    }, [code])
